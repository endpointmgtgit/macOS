...
## Notes
##
## This script can set all existing Admin accounts to be standard user accounts. The account specified in ictadmin will not be downgraded if it is found
##
...
if [[ $downgrade = true ]]; then
  while read useraccount; do
    if [[ "$useraccount" == "ictadmin" ]]; then
        echo "Leaving ictadmin account as Admin"
    else
